...
const STAGE_INFO = {
    Mad_Hatters_Castle: {
        displayedText: "Mad Hatter's Castle",
        color: "#ab0a8c" // 여기 바꿈; 자주
    },
    Caterpillars_Garden: {
        displayedText: "Caterpillar's Garden",
        color: "#36a821" // 여기 바꿈; 초록
    },
    Cheshire_Woods: {
        displayedText: "Cheshire Woods",
        color: "#fcbd14" // 여기 바꿈; 노랑
    },
    The_Looking_Glass: {
        displayedText: "The Looking Glass",
        color: "#480088" // 여기 바꿈; 보라
    },
    Sea_of_Wonder: {
        displayedText: "Sea of Wonder",
        color: "#0249b6" // 여기 바꿈; 파랑
    },
    Queens_Domain: {
        displayedText: "Queen's Domain",
        color: "#ff6b01" // 여기 바꿈; 주황
    }
}
  , SET_TIMES_DATA = {
    Friday: {
  ...
  // 위에 stage_info : 제목 > text, color
  // 위에 set_times_data : 날짜 > 스테이지 > 아티스트 > 시작, 끝 시각
  , END_HOUR = 26                       // 여기 바꿈
  , START_HOUR = 16                       // 여기 바꿈
  ...
  , mapWidth = 200                    // 여기 바꿈
  , mapHeight = 200                   // 여기 바꿈
  , FONT_SIZE_IN_PX = 24              // 24
  , FONT_FAMILIES = "-apple-system, system-ui, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Ubuntu, Times New Roman, serif" 
  ...
  , SITE_PATH = "Made by HeeK" // 여기 바꿈
  , SUNSET_HOUR = 7
  , SUNSET_MINUTES = 7;
var gridColor = "#fff";
const NON_MAIN_STAGES = []
  , EVENT_LENGTH = END_HOUR - 10              // 여기 바꿈
...
function J() {
    ...
    R.lineWidth = 2, // 그리드 선 너비 설정
    R.strokeStyle = gridColor; // 그리드 선 색상 설정
    const U = t=>{ // 시간 형식 함수 정의
        const e = t.split(":");
        var s = parseInt(e[0]) % 12;
        return 0 === s && (s = 12),
        `${s}${"00" == e[1] ? "" : `:${e[1]}`}`
    }
      , H = Math.floor(Math.min(u / 7, G)) * 0.5; // 텍스트 최소 폰트 크기 설정, *0.5 추가
    for (n = 0; n < selectedArtists.length; n++) {
        R.fillStyle = selectedArtists[n].stageColor; // 채우기 설정(무대 색상으로)
        var B = selectedArtists[n].stageNum;
        x || (B = selectedArtists[n].column);
        var b = h + u * B // 아티스트 블록의 x 좌표 계산
          , w = T + _ * (e - (selectedArtists[n].start + selectedArtists[n].duration)) // 아티스트 블록의 y 좌표 계산(공연 종료 시간 기준)
          , k = _ * selectedArtists[n].duration; // 아티스트 블록의 높이 계산(공연 길이)
        R.fillRect(b, w, u, k), // 아티스트 블록 배경 그리기
        R.strokeRect(b, w, u, k), // 아티스트 블록 외곽선 그리기
        R.fillStyle = "#fff"; // 텍스트 색상 설정(횐색)
        var F = Math.min(1.2 * u / selectedArtists[n].name.length, .5 * k, 36) // 아티스트 이름 폰트 크기 계산
          , P = selectedArtists[n].name; // 아티스트 이름
        if (F < H) { // 폰트 크기가 폰트 최소 크기보다 작으면 최소 폰트 크기로 설정
            F = H,
            R.font = Math.min(H, k) + "px " + FONT_FAMILIES; // 폰트 설정, 절단 고려
            const t = .8 * u / R.measureText(P).width // 이름 너비에 맞게 자르기
              , e = Math.floor(P.length * t);
            P = P.substr(0, e) + (e < P.length ? ".." : "") // 너비 초과시 ..으로 표시
        }
        var K = .1 * F;
        const t = Math.min(H, Math.floor((k - H) / 2)) // 공연 시간 길이에 따른 텍스트 중앙 정렬 간격 계산
          , s = k > H && t > 6; // 공연 시간이 길고, 간격을 둘 수 있는 경우
        R.font = F + "px " + FONT_FAMILIES, // 텍스트 폰트 설정
        R.textBaseline = "middle", // 텍스트 기준선 설정
        R.fillText(P, b + u / 2, s ? w + k / 2 - t / 2 : w + k / 2), // 아티스트 이름
        R.textBaseline = "middle", // 텍스트 기준선 설정
        R.font = `500 ${t}px ${FONT_FAMILIES}`, // 공연 시간 정보 폰트 설정
    ...
    const z = Math.round(.15 * Y);             // 이거 S A T U R D A Y
    R.font = `900 ${z}px ${FONT_FAMILIES}`;    // 900 
    var q = selectedDay.toUpperCase().split("").join("  ");
    R.fillText(q, .055 * Y, V);
  
    const tt = STAGE_NAMES.length              // 이거 무대 이름
      , et = V + z + 4
      , st = (T - (et + V) - 4 * (tt - 1)) / tt
      , nt = Math.round(.15 * Y);
    R.font = `600 ${nt}px ${FONT_FAMILIES}`, // 700 -> 600 
    ...
    R.font = `500 ${FONT_SIZE_IN_PX}px ${FONT_FAMILIES}`; // 500
