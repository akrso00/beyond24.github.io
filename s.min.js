...
function J() {
    ...
    const U = t=>{ // 시간 형식 함수 정의
      ...
      , H = Math.floor(Math.min(u / 7, G)) * 0.7; // 텍스트 최소 폰트 크기 설정, *0.7 추가
    for (n = 0; n < selectedArtists.length; n++) {
        ...
        R.fillText(P, b + u / 2, s ? w + k / 2 - t / 2 : w + k / 2), // 아티스트 이
        R.textBaseline = "middle", // 텍스트 기준선 설정
    ...
    const tt = STAGE_NAMES.length
      ...
      , st = (T - (et + V) - 4 * (tt - 1)) / tt
      , nt = Math.round(.15 * Y);
    R.font = `700 ${nt}px ${FONT_FAMILIES}`, // 700
...
let stage_column_width = (screen_width / STAGE_NAMES.length) - 30; // () - 30 추가
var Q;
function X(t) {
